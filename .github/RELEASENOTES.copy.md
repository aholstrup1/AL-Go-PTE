...
### Add custom steps to the AL-Go build workflow

It is now possible to add custom steps to the AL-Go for GitHub `BuildALGoProject` job in the `_BuildALGoProject.yaml` file. Custom steps needs to be named `CustomStep<something>` and can be placed at one of a set of predefined anchors. See [https://aka.ms/algosettings#customsteps](https://aka.ms/algosettings#customsteps) for details.

Note that custom steps might break by future changes to AL-Go for GitHub workflows. If you have customized steps to AL-Go for GitHub workflows, you should always doublecheck the pull request generated by Update AL-Go System Files.

### Indirect AL-Go template repository
